...
\item{Stage A}: Compute an orthornormal low rank basis $\bold{Q}$ such that, \\
  $\bold{A} \approx \bold{Q}\bold{Q}^TA$
...
Given $\bold{A}$ and $k$, seek $\bold{B}$ st
$$\lVert \bold{A} - \bold{Q}\bold{Q}^T \lVert \approx \min_{rank(\bold{B}) \leq
...
\frame{
\begin{block}{Example 2 - Large sparse Matrix}
Matrix formed computing the graph Laplacian on a small patch of Lena
\end{block}
